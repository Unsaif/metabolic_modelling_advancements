...
def verify_signatures(case, legacy, target):
    require(legacy == target, 'Independent explicit preparation changes the legacy numeric LP or chemistry')
    require(case['legacy_signature_sha256'] == json_digest(legacy), 'Saved legacy signature differs from reconstruction')
    require(case['algebra_and_chemistry_sha256'] == json_digest(target), 'Saved prepared signature differs from reconstruction')
    changed_types = []
    def walk(before, after, path):
        if isinstance(before, dict):
            ...
            for index, (a, b) in enumerate(zip(before, after)):
                walk(a, b, path + [index])
        elif type(before) != type(after):
            require(finite(before) and finite(after) and before == after, 'Unexpected signature representation difference')
            changed_types.append({'path': path, 'legacy_type': type(before).__name__,
                                  'prepared_type': type(after).__name__, 'value': before})
    walk(legacy, target, [])
...
def verify_partial(run):
    ...
    for item in plan['models']:
        ...
        for condition in selected:
            ...
            representation = verify_signatures(case, signature, canonical(target))
            if representation:
                serialization.append({'model': label, 'condition': condition.key, 'equal_numeric_value_type_changes': representation})
    ...
    return {'status': 'passed_partial', 'coverage': 'Only saved cases verified; the 156-case study remains incomplete.',
            ...
            'numeric_serialization_differences': serialization,
...
def verify(run):
    ...
    inventory, outcomes, serialization = set(), [], []
    maxima = {'S_residual': 0., 'bound_violation': 0., 'solver_objective_difference': 0.}
    ...
    for item in plan['models']:
        ...
        for condition in conditions:
            ...
            target_signature = canonical(target)
            representation = verify_signatures(case, signature, target_signature)
            if representation:
                serialization.append({'model': label, 'condition': condition.key, 'equal_numeric_value_type_changes': representation})
            check_report(case['medium_report'], expected_report)
            if key in partial:
                prior = partial[key]
                verify_signatures(prior, signature, target_signature)
