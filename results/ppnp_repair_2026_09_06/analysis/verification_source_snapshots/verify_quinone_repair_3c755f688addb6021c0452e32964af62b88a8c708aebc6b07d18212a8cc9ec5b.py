...
def verify_model_provenance(provenance, directory, model):
    """Allow a relocated checkout while retaining arm and content identity.

    The card records its original execution path. Its checkout prefix is
    historical provenance, not a requirement for reading committed artifacts
    on another machine. The expected arm/filename and actual bytes are required.
    """
    arm = directory.name
    recorded = provenance['file']
    require(isinstance(recorded, str) and Path(recorded).parts[-2:] == (arm, 'model.xml.gz'),
            arm + ' model provenance names the wrong arm or filename')
    for key, value in [('n_reactions', len(model.reactions)), ('n_metabolites', len(model.metabolites)),
                       ('n_genes', len(model.genes)), ('model_id', model.id),
                       ('sha256', sha(directory / 'model.xml.gz'))]:
        equal(provenance[key], value, arm + ' model provenance ' + key)
...
def verify_benchmark(directory, report, recipe, manifest, fitness, experiments, conditions=None):
    ...
    verify_model_provenance(card['model'], directory, model)
...
def main():
    ...
    report = {'created_utc': datetime.now(timezone.utc).isoformat(), 'passed': False,
              ...
              'model_path_policy': 'Original card checkout prefixes are retained as provenance; arm/filename suffix and current exported model hash/identity are verified, allowing relocation.',
