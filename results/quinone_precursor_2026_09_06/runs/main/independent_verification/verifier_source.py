...
def network_from_model(model):
    ...
    return records, {met.id for met in model.metabolites}, {r.id for r in model.exchanges}
...
def prepare_network(base, metabolites, exchanges, conditions, media, condition, recipe):
    ...
    records = dict(base)
    metabolites, exchanges = set(metabolites), set(exchanges)
    added = []
...
def verify_case(row, expected, records, base_bounds, aliases, recipe, epsilon):
    ...
    return {'case': expected, 'solvers': checks, 'objective_disagreement': abs(first['objective'] - second['objective'])}
