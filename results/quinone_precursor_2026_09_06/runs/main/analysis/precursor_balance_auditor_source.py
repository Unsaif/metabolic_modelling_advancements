...
def exact_rows(path):
    ...
    return rows, bounds, epsilon, set(reactions)
...
def analyze(run):
    ...
    files += [parent, Path(__file__).resolve()]
    ...
    rows, bounds, epsilon, reaction_ids = exact_rows(parent)
    ...
    # Without new stoichiometry, bounds and gene deletions preserve every row.
    # A nonempty completion requires a separate explicit stoichiometric audit.
    require(all(not r['medium_completion_added'] for r in contexts),
            'Medium completion added reactions; independent row applicability needs their stoichiometry')
    ...
    return {'status': 'passed', 'scope': 'Exact source-row derivation and saved-flux interpretation; no optimization or phenotype values.',
            ...
            'all_contexts_have_no_stoichiometric_completion': True,
