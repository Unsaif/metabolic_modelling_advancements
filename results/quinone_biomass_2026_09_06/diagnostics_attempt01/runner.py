...
def independent_summary(study):
    ...
    prior_paths = {
        ...
        "archived_cycle7": ROOT / "results/carbon_fitness_multi/Putida/Pseudomonas_putida_KT2440_xml_gapfilled__gapfilled__patched-v0.1+model-v0.3+gpr-v0.2+0.3+0.4+medium__nodroprich/matrices.npz",
    }
    comparisons = []
    for name, path in prior_paths.items():
        ...
        record = {"comparison": name, "available": True, "path": str(path.relative_to(ROOT)),
                  "sha256": sha256(path), "same_gene_condition_labels": same_labels}
        if same_labels:
            record.update(max_abs_wt_difference=float(np.max(np.abs(base["wt_growth"] - old["wt_growth"]))),
                          max_abs_ko_difference=float(np.max(np.abs(base["sim_growth"] - old["sim_growth"]))),
                          n_changed_binary_ko_predictions=int(((base["sim_growth"] >= threshold) != (old["sim_growth"] >= threshold)).sum()),
                          same_experimental_fitness=bool(np.array_equal(base["fitness"], old["fitness"], equal_nan=True)))
