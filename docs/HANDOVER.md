...
7. B. thetaiotaomicron glycan/PUL-aware mapping (22 of 47 carbon sources unmapped), DepMap comparison, Bernstein reconciliation (Q7), and — new — a proposal upstream to the EMBL GEMs / CarveMe maintainers about the ATP synthase omission (a one-line addition per model once the operon is located; the survey script and sample list make the case).
