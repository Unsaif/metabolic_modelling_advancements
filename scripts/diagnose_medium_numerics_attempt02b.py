...
def main():
    ...
    try:
        ...
        # SBML normalizes nested OR strings. Preserve the strict GPR string
        # contract by reconstructing from the same immutable original sources.
        inventory = R.inventory_module()
        base, provenance = inventory.build_parent(first_settings['model_label'])
        recipe = R.read(R.PLAN)
        entry = next(item for item in recipe['models'] if item['label'] == first_settings['model_label'])
        conditions = inventory.metadata_conditions('Putida')
        condition = conditions[0]
        D.require(condition.key == first_settings['condition'], 'First curated condition changed')
        completion_media = [R.base_medium(name) for name in sorted({item.media for item in conditions})]
        prepared = R.prepare_medium(base, R.medium_for(condition), policy=R.CompartmentPolicy(**entry['compartments']),
        ...
        D.require(R.signature(model) == signature, 'Original-source reconstruction does not reproduce exact LP/GPR/chemistry')
        D.write(out / 'prepared_context.json', {'source_model': provenance, 'preparation': 'Rebuilt from frozen original sources; SBML GPR string normalization not used',
