...
def aucpr_b(sim, fit, st=1e-3):
    y = (sim >= st).astype(int).ravel(); s = -fit.ravel(); ok = np.isfinite(s) & np.isfinite(sim.ravel())
    if not ok.any() or y[ok].min() == y[ok].max():
        return float("nan")
...
def main() -> None:
    ...
    args = ap.parse_args()
    problems = 0
    cards = sorted(glob.glob(os.path.join(args.results_dir, "*", "*", "card.json")))
    if not cards:
        raise SystemExit("No benchmark cards found; nothing was verified")
    for card_path in cards:
        ...
        if s2.size:
            a = aucpr_b(s2, f2, st)
            okm = np.isfinite(f2.ravel()) & np.isfinite(s2.ravel())
            pred = (s2.ravel() >= st).astype(int)[okm]; lab = (f2.ravel() >= ft).astype(int)[okm]
            m = matthews_corrcoef(lab, pred) if okm.any() else float("nan")
            line += f" | AUC-PR {a:.3f} (card {rep['aucpr_bernstein']['point']:.3f}) MCC {m:.3f} (card {rep['mcc']['point']:.3f})"
            if not (np.isclose(a, rep["aucpr_bernstein"]["point"], atol=1e-9, rtol=0, equal_nan=True)
                    and np.isclose(m, rep["mcc"]["point"], atol=1e-9, rtol=0, equal_nan=True)):
        ...
        for _ in range(3):
            ...
            if not np.isclose(v, fit[i, j], atol=1e-9, rtol=0, equal_nan=True):
        ...
        if not np.isfinite(sim).all() or not np.isfinite(wt).all():
            line += "  NONFINITE-SIMULATION"; problems += 1
