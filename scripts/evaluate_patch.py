...
def main() -> None:
    ...
    base_ver = sys.argv[2] if len(sys.argv) > 2 else None      # compare against another patched version instead of the unpatched run
    rows, changes = [], []
    for pd_dir in sorted(glob.glob(os.path.join(RES, "*", f"*__patched-{ver}"))):
        ...
        row = {"org": org, "model": label, "genes_compared": len(common), "genes_dropped_by_rich_medium_step": ";".join(dropped),
               ...
               "pairs_changed_to_essential": int(newly_ess.sum()), "to_essential_experimentally_important": int((newly_ess & imp).sum()),
               "pairs_changed_to_dispensable": int(newly_disp.sum()), "to_dispensable_experimentally_important": int((newly_disp & imp).sum())}
    ...
    tag = f"{ver}_vs_{base_ver}" if base_ver else ver
    df.to_csv(os.path.join(RES, f"patch_{tag}_evaluation.tsv"), sep="\t", index=False)
    ch.to_csv(os.path.join(RES, f"patch_{tag}_gene_changes.tsv"), sep="\t", index=False)
